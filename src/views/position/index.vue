<template>
    <div class="absolute top-0 left-0 right-0 flex flex-col">
      ...
      <div class="p-3 pb-[50px] text-[1rem]">
        ...
        <div v-for="(item,index) in orders" :key="item.id" class="flex flex-col bg-[#32373A] p-5 gap-2 rounded my-[15px]">
          <div class="flex justify-between items-center">
            <div>{{item.symbol.displayName}} [<span v-if="item.dir" class="textDanger"> {{ $t('buy') }} </span> <span v-else class="textSuccess"> {{ $t('sell') }} </span>]</div>
            <div  v-if="item.status==0" class="text-red-500">{{ $t('openposition') }}</div>
            <div  v-else class="text-green-500">{{ $t('closeposition') }}</div>
          </div>
          <div class="flex justify-between items-center">
            <div>
              {{ $t('orderbalance') }}: {{item.money}}
            </div>
            <div>
              {{ $t('profit') }}:
              ...
              <span v-else-if="item.status==1&&item.during==600" class="textDanger"> -{{Number(0.01*Number(item.money)*Number(item.symbol.lossRatio.split(',')[2])).toFixed(2)}}</span>
          ...
          <div class="flex justify-between items-center">
            <div  v-if="item.lastprice!='-'">
              {{ $t('openprice') }}: {{item.price}} - {{Number(item.lastprice).toFixed(2)}}
            </div>
            <div  v-else>
              {{ $t('openprice') }}: {{item.price}} - ?
            </div>
            <div>
              {{ $t('toll') }}: {{getSystem.bettingPercent}}
            </div>
          </div>
          <div class="flex justify-between items-center">
            <div >
